...
# Load model.
model = Autoencoder()
model.load_state_dict(torch.load('best_model_denoising_autoencoder.pth'))

# Obtain encoded representation of samples.
data, label = next(iter(loader))
label = label.numpy()

data = Variable(add_white_noise(data))
...
plt.title('t-SNE Visualization of Noisy Fashion MNIST')
plt.xlabel('x in t-SNE')
plt.ylabel('y in t-SNE')
plt.legend()
plt.savefig('img/t_sne_noisy_fashion_mnist.png')
