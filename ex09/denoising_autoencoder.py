...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        data, noisy_data, target = Variable(data), Variable(noisy_data), Variable(target)
        optimizer.zero_grad()
...
def test():
    ...
    for batch_idx, (data, target) in enumerate(test_loader):
        noisy_data = add_white_noise(data)
        data, noisy_data, target = Variable(data, volatile=True), Variable(noisy_data, volatile=True), Variable(target)
        output = model(noisy_data)
        ...
        if batch_idx % args.test_batch_size == batch_idx:
            ...
            return test_loss.data[0], \
                   ...
                   noisy_data[0].data.numpy().reshape(input_shape), \
...
fig_encoder.suptitle('Denoising Autoencoder')
...
for epoch in range(1, args.epochs + 1):
    ...
    test_loss, input, noisy_input, output = test()
    ...
    if epoch % 10 == 0:
        ...
        plt.subplot(133)
        plt.title('Denoised at Epoch {}'.format(epoch))
        plt.imshow(output)

        plt.savefig('img/denoising_autoencoder_epoch_{}.png'.format(epoch))

    if float(test_loss) < lowest_loss:
        print('New lowest loss achieved: {:.2f}%\n'.format(float(test_loss)))
        lowest_loss = test_loss
        torch.save(model.state_dict(), 'best_model_denoising_autoencoder.pth')
...
plt.savefig('img/loss_progress_denoise.png')
